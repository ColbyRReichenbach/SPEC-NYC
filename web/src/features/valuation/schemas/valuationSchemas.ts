...
export const singleValuationRequestSchema = z.object({
  property: z.object({
...
export const singleValuationResponseSchema = z.object({
...
export const propertyExplanationRequestSchema = z.object({
  valuation_id: z.string().min(1),
  top_n: z.number().int().min(1).max(10).default(5)
});

export const propertyExplanationResponseSchema = z.object({
  valuation_id: z.string(),
  status: z.enum(["ready", "degraded", "unavailable"]),
  explainer_type: z.string(),
  drivers_positive: z.array(driverSchema),
